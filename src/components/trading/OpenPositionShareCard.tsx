...
export interface OpenPositionShareCardProps {
  market: string;
  side: 'Long' | 'Short';
  entryPrice: number;
  markPrice: number;
...
export interface OpenPositionOverlayCaptureProps {
  market: string;
  side: 'Long' | 'Short';
  entryPrice: number;
  markPrice: number;
...
/** Shared overlay content used both inside OpenPositionShareCard and for the hidden GIF-capture node */
    ...
    <>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <div style={{ fontSize: '14px', fontWeight: 600, letterSpacing: '0.05em', opacity: 0.7, color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
              OPEN POSITION
            </div>
            <div style={{ fontSize: '22px', fontWeight: 800, marginTop: '2px', color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>{market}</div>
          </div>
        </div>
        ...
        >
          {side}{leverage != null ? ` ${leverage}x` : ''}
        </span>
      </div>

      {/* Center: giant unrealized PnL — shows either $ or % based on pnlMode */}
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'baseline', gap: '16px' }}>
        <div style={{ fontSize: '72px', fontWeight: 800, lineHeight: 1, color: pnlColor, WebkitTextFillColor: pnlColor, WebkitTextStroke: '0' }}>
          {pnlMode === '%'
            ? (pnlPercent != null ? formatPct(pnlPercent) : formatPnl(unrealizedPnl))
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', gap: '40px', marginBottom: '20px' }}>
          <div>
            <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.1em', opacity: 0.5, marginBottom: '4px', color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
              ENTRY PRICE
            </div>
            <div style={{ fontSize: '18px', fontWeight: 700, color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
          ...
          {liquidationPrice != null && isFinite(liquidationPrice) && (
            <div>
              <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.1em', opacity: 0.5, marginBottom: '4px', color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
                LIQ PRICE
              </div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#b794f6', WebkitTextFillColor: '#b794f6', WebkitTextStroke: '0' }}>
                ${formatPriceCompact(liquidationPrice)}
              </div>
            </div>
          )}
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.04em', color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
              aeonian.trade
              <span style={{ opacity: 0.45, fontWeight: 500, color: fontColor, WebkitTextFillColor: fontColor }}> · Solana Perps</span>
            </span>
            {xUsername && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 600, color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0', opacity: 0.75 }}>
                  @{xUsername}
                </span>
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
            ...
            <span style={{ fontSize: '11px', opacity: 0.4, fontWeight: 500, color: fontColor, WebkitTextFillColor: fontColor, WebkitTextStroke: '0' }}>
...
const OpenPositionShareCard = forwardRef<HTMLDivElement, OpenPositionShareCardProps>(
  ({ market, side, entryPrice, markPrice, size, unrealizedPnl, leverage, liquidationPrice, bgColor, fontColor, logoUrl, platformLogoUrl, bgImage, pnlMode = '$', xAvatarDataUrl, xUsername }, ref) => {
...
/**
 * Standalone hidden overlay capture node for GIF compositing.
 ...
 */
export function OpenPositionOverlayCapture({
  market,
  side,
  entryPrice,
  markPrice,
  ...
  xAvatarDataUrl,
  xUsername,
}: OpenPositionOverlayCaptureProps) {
  const isPositive = unrealizedPnl >= 0;
  const pnlColor = isPositive ? '#4ADE80' : '#FF5252';
  const pnlPercent =
    entryPrice > 0 && size > 0
...
OpenPositionShareCard.displayName = 'OpenPositionShareCard';

export default OpenPositionShareCard;
