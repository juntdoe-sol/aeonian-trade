...
import { Check, Copy, ExternalLink, LifeBuoy, LogOut, Sparkles, TrendingUp, Vibrate, Volume2, VolumeX } from 'lucide-react';
...
import { useCelebrationStore } from '@/utils/celebrate';
...
export function AccountPage() {
  ...
  const { enabled: celebrationEnabled, setEnabled: setCelebrationEnabled } = useCelebrationStore();
      ...
      <div className='hidden lg:block'>
        <div className='max-w-6xl mx-auto px-6 pt-6 pb-10'>
          <div className='flex gap-8 items-start'>
            {/* LEFT COLUMN: trading account metrics */}
            <div className='flex-shrink-0 space-y-4' style={{ width: '420px' }}>
              {/* Wallet card */}
                ...
                {user ? (
                  <>
                    <div className='flex items-center justify-between'>
                      <div>
                        <div className='text-xs mb-1' style={{ color: '#8A8A8A' }}>Connected</div>
                        <div className='font-mono text-base font-bold' style={{ color: '#b794f6' }}>
                    ...
                    <div className='glass-inner rounded-lg p-3 font-mono text-xs break-all' style={{ color: '#8A8A8A' }}>
                      {user.address}
                    </div>
                    <button onClick={logout} className='glass-button w-full flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-medium transition-colors' style={{ color: '#FF5252' }}>
                      <LogOut size={15} />
                      Log Out
                    </button>
                  </>
              ...
              {user && (
                <div className='glass-card rounded-xl p-5 space-y-4'>
                  <div className='flex items-center justify-between'>
                    ...
                    <button onClick={() => navigate('/portfolio')} className='text-xs font-medium transition-colors' style={{ color: '#b794f6' }}>
                      View Portfolio →
                    </button>
                  </div>
                  <ActiveAccountFlow open={activateOpen} onOpenChange={setActivateOpen} />
                  {traderLoading ? (
                    <div className='h-20 rounded-xl animate-pulse' style={{ background: '#0A0A0A' }} />
                  ) : isRegistered === false ? (
                    <div className='text-sm py-2' style={{ color: '#8A8A8A' }}>
                      No active Phoenix account.{' '}
                      <button onClick={() => setActivateOpen(true)} className='font-medium' style={{ color: '#b794f6' }}>Activate →</button>
                    </div>
                  ) : isRegistered && trader ? (
                    <>
                      {/* Larger metrics at desktop */}
                      <div className='grid grid-cols-3 gap-3'>
                        <div className='glass-inner rounded-xl p-4 text-center'>
                          <div className='text-xs mb-2' style={{ color: '#8A8A8A' }}>Total Collateral</div>
                          <div className='font-bold text-lg tabular-nums'>{formatUsd(toNumber(trader.collateralBalance))}</div>
                        </div>
                        <div className='glass-inner rounded-xl p-4 text-center'>
                          <div className='text-xs mb-2' style={{ color: '#8A8A8A' }}>Withdrawable</div>
                          <div className='font-bold text-lg tabular-nums' style={{ color: '#4ADE80' }}>{formatUsd(toNumber(trader.effectiveCollateralForWithdrawals))}</div>
                        </div>
                        <div className='glass-inner rounded-xl p-4 text-center'>
                          <div className='text-xs mb-2' style={{ color: '#8A8A8A' }}>Unrealized PnL</div>
                          <div className='font-bold text-lg tabular-nums' style={{ color: toNumber(trader.unrealizedPnl) >= 0 ? '#4ADE80' : '#FF5252' }}>
                    ...
                    <div className='text-sm py-2' style={{ color: '#8A8A8A' }}>Account registered — live trading data will appear on mainnet.</div>
                  ) : null}
                </div>
              )}
              ...
              <div className='glass-card rounded-xl px-5 py-4 flex items-center justify-between'>
                ...
                <div className='flex items-center gap-1.5'>
                  ...
                  <span className='text-sm font-medium tabular-nums' style={{ color: '#E5E5E5' }}>Solana Mainnet</span>
                </div>
              </div>

              {/* Preferences */}
              <div className='glass-card rounded-xl p-5 space-y-3'>
                <h3 className='text-xs font-medium uppercase tracking-wider' style={{ color: '#8A8A8A' }}>Preferences</h3>
                {/* Sound effects */}
                <div className='flex items-center justify-between gap-4 py-1'>
                  <div className='flex items-center gap-3 flex-1 min-w-0'>
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='text-xs mt-0.5' style={{ color: '#8A8A8A' }}>Button clicks and win chime.</div>
                    </div>
                  </div>
                ...
                <div className='flex items-center justify-between gap-4 py-1'>
                  <div className='flex items-center gap-3 flex-1 min-w-0'>
                    <Sparkles size={16} style={{ color: celebrationEnabled ? '#b794f6' : '#8A8A8A', flexShrink: 0 }} />
                    <div className='flex-1 min-w-0'>
                      <div className='font-semibold text-sm' style={{ color: '#E5E5E5' }}>Win celebration</div>
                      <div className='text-xs mt-0.5' style={{ color: '#8A8A8A' }}>Coin-rain animation on profitable closes.</div>
                    </div>
                  </div>
                  <Switch checked={celebrationEnabled} onCheckedChange={setCelebrationEnabled} />
                </div>
                <div style={{ height: 1, background: 'rgba(255,255,255,0.06)' }} />
                {/* Haptic feedback */}
                <div className='flex items-center justify-between gap-4 py-1'>
                  <div className='flex items-center gap-3 flex-1 min-w-0'>
                    <Vibrate size={16} style={{ color: hapticsEnabled ? '#b794f6' : '#8A8A8A', flexShrink: 0 }} />
                    <div className='flex-1 min-w-0'>
                      <div className='font-semibold text-sm' style={{ color: '#E5E5E5' }}>Haptic feedback</div>
                      <div className='text-xs mt-0.5' style={{ color: '#8A8A8A' }}>Vibrate on button taps (mobile only).</div>
                    </div>
                  </div>
              ...
              <div className='glass-card rounded-xl p-5 space-y-3'>
                ...
                <div className='flex items-center justify-between gap-3'>
                  <div className='glass-inner flex-1 font-mono text-xs break-all select-all px-3 py-2.5 rounded-lg' style={{ color: '#C0C0C0' }}>
                    {TIP_ADDRESS}
                  </div>
                  <button onClick={copyTipAddress} className='glass-button flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg text-xs font-medium transition-colors flex-shrink-0' style={{ color: tipCopied ? '#4ADE80' : '#8A8A8A' }}>
            ...
            {/* RIGHT COLUMN: FAQ + support + about */}
            <div className='flex-1 min-w-0 space-y-4'>
              {/* About AEONIAN */}
              <div className='glass-card rounded-xl p-5 space-y-4'>
                <div className='flex items-center gap-3'>
                  <img src={NEW_LOGO_URL} alt='AEONIAN' style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 8 }} />
                  <h3 className='font-bold text-lg'>About AEONIAN</h3>
                </div>
                ...
                <a href='https://phoenix.trade' target='_blank' rel='noopener noreferrer' className='flex items-center gap-1.5 text-sm font-medium transition-colors' style={{ color: '#b794f6' }}>
                  phoenix.trade
              ...
              <div className='glass-card rounded-xl p-5 space-y-3'>
                ...
                <a href='https://x.com/i/chat/group_join/g2057103803988742289/sf0fKNVDJo' target='_blank' rel='noopener noreferrer' className='glass-inner flex items-center justify-between w-full rounded-lg px-4 py-4 transition-colors'>
                  <div className='flex items-center gap-3'>
                    <LifeBuoy size={18} style={{ color: '#b794f6' }} />
                    <div>
                  ...
                  <ExternalLink size={14} style={{ color: '#8A8A8A', flexShrink: 0 }} />
                </a>
              </div>

              {/* FAQ */}
              <div className='glass-card rounded-xl p-5 space-y-1'>
                <h3 className='text-xs font-medium uppercase tracking-wider mb-4' style={{ color: '#8A8A8A' }}>FAQ</h3>
                <Accordion type='single' collapsible className='w-full'>
                  {[
                    { id: 'q1', q: 'What is Phoenix Perps?', a: 'Phoenix Perps is a leveraged perpetual futures trading platform built on Phoenix Exchange — one of the fastest on-chain central limit order book (CLOB) DEXes on Solana. Trade SOL, BTC, ETH, and more with up to 20x leverage.' },
                    { id: 'q2', q: 'How do deposits and withdrawals work?', a: 'Deposit USDC from your connected Solana wallet into your Phoenix trading account. Withdrawals return USDC to your wallet and are subject to margin requirements — you can only withdraw collateral not locked by open positions.' },
                    { id: 'q3', q: "Why isn't my trade going through?", a: 'Common causes include insufficient collateral, exceeding max leverage, low SOL balance for transaction fees, or a slow network. Make sure your account is activated and funded. If the issue persists, reach out in our Support Chat Group.' },
                    { id: 'q4', q: 'Is this available in the US?', a: 'Perpetual futures trading via Phoenix Exchange is not available to US residents due to regulatory restrictions. Access to trading features is automatically restricted based on your location.' },
                    { id: 'q5', q: 'How do I activate my trading account?', a: 'Connect your Solana wallet, then tap "Activate" in the Active Account section. Activation registers your wallet with Phoenix Exchange and is required before you can deposit or trade.' },
                    { id: 'q6', q: 'Where do I get help?', a: 'Join our Support Chat Group on X (link above) to report bugs, ask questions, or share feedback. You can also visit phoenix.trade for protocol-level documentation.' },
                  ].map(({ id, q, a }) => (
                    <AccordionItem key={id} value={id} style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                      <AccordionTrigger className='text-sm font-medium py-3.5 hover:no-underline' style={{ color: '#E5E5E5' }}>{q}</AccordionTrigger>
                      <AccordionContent className='text-sm pb-3.5' style={{ color: '#8A8A8A' }}>{a}</AccordionContent>
                    </AccordionItem>
                  ))}
                </Accordion>
              </div>
